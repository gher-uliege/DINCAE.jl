...
sz = (6,7)
A = DINCAE.interp_adjn(pos,values,sz)
...
cu_A = DINCAE.interp_adjn(cu_pos,cu_values,sz)
...
A = ones(Float32,sz)
f(pos,A) = sum(DINCAE.interpnd(pos,A))


dA = gradient(f,pos,A)[2]
cu_dA = gradient(f,cu_pos,cu_A)[2]
